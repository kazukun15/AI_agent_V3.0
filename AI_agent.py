...
def cached_get_search_info(query: str) -> str:
    # 仮の tavily API エンドポイント（実際の仕様に合わせて変更）
    ...
    try:
         ...
         data = response.json()
         # tavily API のレスポンス仕様に合わせて "summary" キーを取得する例
...
@st.cache_data(show_spinner=False)
def cached_get_search_info(query: str) -> str:
    # 仮の tavily API エンドポイント（実際の仕様に合わせて変更してください）
    url = "https://api.tavily.com/v1/search"
    params = {
         "q": query,
         "format": "json",
    ...
    try:
         response = requests.get(url, params=params)
         data = response.json()
         # tavily API の仕様に合わせ、"summary" キーから取得する例
         result = data.get("summary", "")
         return result
    except Exception as e:
         return ""
